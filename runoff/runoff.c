...
bool vote(int voter, int rank, string name)
{
    for (int i = 0; i < candidate_count; i++)
    {
        ...
        if (strcmp(candidates[i].name, name) == 0)
        {
            ...
            // printf("Voter: %i Rank: %i Candidate: %s\n", voter, rank, candidates[i].name);
...
bool print_winner(void)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        int percentage = ((float) candidates[i].votes / voter_count) * 100;
        if (percentage > 50)
        {
            printf("%s\n", candidates[i].name);
            return true;
...
bool is_tie(int min)
{
    for (int i = 0; i < candidate_count; i++)
    {
        if (candidates[i].eliminated == false && candidates[i].votes > min | candidates[i].votes < min)
        {
            return false;
        }
    }
